...
On **profile** pages, the scripture modal uses **`GET /api/scripture/spurgeon-links?reference=…`** while the modal is open (with **`onOpenSpurgeonStudy`** wired) to see if any indexed public sermon cites that passage. **Study** stays in the toolbar: it is **enabled** when that response includes at least one `{ slug, title }`, and **disabled** (greyed out) while loading, on error, or when there are no matches. Clicking **Study** when enabled opens the **Spurgeon sermons** modal on the **By scripture** tab with the current passage filled in and **`GET /api/spurgeon/by-reference`** run so **all** matching sermon templates (`sg…` slugs) appear in one list. The reader shows **verse text only** (no inline presentation subsection HTML). The reader opens on **verse** view (not full-chapter context).
