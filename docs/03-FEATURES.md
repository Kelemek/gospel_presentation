...
- **Memorize**: Toolbar **Memorize** (`data-tour="scripture-modal-memorize"`) saves the passage to the local memorization list when text has loaded; disabled while loading, on error, if already saved, or if there is no text. See **Verse memorization** below.
...
- **Add from scripture modal**: When the **Scripture** modal is open (`ScriptureModal.tsx`), a **Memorize** control in the header saves the current passage (reference + plain text + selected translation) to **`localStorage`** (`gospel-memorization-verses` via `verseMemorizationStorage.ts`). HTML and verse markers like `[16]` are stripped before storage. Duplicates (same reference + translation) are rejected; success uses the standard alert modal.
...
- **Practice session** (`MemorizationPracticeSession.tsx`): Starts with the full verse, then **Start practice** begins five rounds with increasing share of words hidden (20% … 100%). Which words are blanked is randomized **per practice run** (new randomness each time you tap **Start practice**); **Repeat this round** / **Next round** keep the same blanks for that run (`memorizationPracticeUtils.ts`). For each blank, type the **first letter** of the word (case-insensitive); correct input reveals the word and advances. A nearly invisible **text field** stays in the DOM from intro through practice; **Start practice**, **Repeat this round**, and **Next round** use **`flushSync` + `focus()`** in the same tap handler so **iOS Safari and Capacitor WebView** treat focus as user-initiated and show the **system keyboard**. The practice panel adds **bottom padding** from **`window.visualViewport`** when the keyboard shrinks the visible area, and **scrolls the verse block** toward the center on focus so the passage stays above the keyboard. **Hint** does not blur the field so the keyboard stays up while peeking. Three wrong letters in a row for the same blank auto-reveal that word. **Hint**: hold the **Hint** button to temporarily peek at still-hidden words in order (italic blue styling): the **first** blank appears immediately, then **one more blank every 3 seconds** while you keep holding; release to hide them again—keyboard input is ignored while the hint is held. After rounds 1–4 complete, **Repeat this round** and **Next round** appear in a **footer** at the bottom of the same modal while the verse stays on screen, with a **random affirmation** above the buttons (`memorizationEncouragementMessages.ts`: treasuring God’s Word in the heart, perseverance). After **round 5**, a **random congratulation** message is shown (same module) before **Done**; completing round 5 records a practice session (wrong attempts, correct keystrokes) and updates stats in localStorage. **Escape** closes the overlay; body scroll is locked while the session is open.
- **Tutorial** (**Help** → **Verse memorization**): Opens a scripture card, saves with **Memorize** in the reader (toast OK uses `data-tour="alert-modal-ok"`), closes the reader, opens **Menu** → **Memorize**, explains the list, then removes that verse with confirm (`tryStartMemorizeTourAfterNavigation` on `/default` when starting from another profile).
