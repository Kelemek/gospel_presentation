...
function subsectionCount(parsed: ParsedMcheynePlan): number {
  return parsed.gospelData.reduce((n, sec) => n + (sec.subsections?.length ?? 0), 0)
}

/** Upsert M'Cheyne reading plan (`mchy`) and rebuild `spurgeon_passage_index`. */
export async function importMcheyneToSupabase(
  supabase: SupabaseClient,
  parsed: ParsedMcheynePlan
): Promise<{
  slug: string
  ...
  return {
    ...
    subsectionCount: subsectionCount({ ...parsed, gospelData }),
  }
}
