...
import { parseReferenceMemorizationTokens } from '@/lib/memorizationPracticeUtils'
...
/** Single-token STT homophones for uncommon biblical words. */
const RECITE_SPOKEN_WORD_ALIASES: Record<string, readonly string[]> = {
  ascents: ['assents'],
};

/** Article + word pairs STT splits into instead of one expected token (Ascents → a sense). */
const RECITE_ARTICLE_HOMOPHONE_PHRASES: Record<string, readonly (readonly [string, string])[]> = {
  ascents: [['a', 'sense']],
};

function reciteSpokenAliasMatches(expected: string, spoken: string): boolean {
  const aliases = RECITE_SPOKEN_WORD_ALIASES[expected];
  if (!aliases) return false;
  return aliases.some((alias) => alias === spoken || wordsFuzzyMatch(expected, alias));
}

function tryArticleHomophonePhrase(
  spoken: string[],
  spokenIdx: number,
  expectedWord: string
): { heard: string; wordsConsumed: number } | null {
  if (spokenIdx + 1 >= spoken.length) return null;
  const phrases = RECITE_ARTICLE_HOMOPHONE_PHRASES[expectedWord];
  if (!phrases) return null;
  const w0 = spoken[spokenIdx]!;
  const w1 = spoken[spokenIdx + 1]!;
  for (const [article, tail] of phrases) {
    if (w0 === article && normalizeReciteWord(w1) === normalizeReciteWord(tail)) {
      return { heard: `${w0} ${w1}`, wordsConsumed: 2 };
    }
  }
  return null;
}
...
function tokenMatchStatus(
  ...
  if (reciteSpokenAliasMatches(expected, spoken)) return 'correct';
...
function alignTypableSubsequence(
  ...
  for (let slot = 0; slot < typableIndices.length; slot++) {
    ...
    while (spokenIdx < spoken.length || digitRemainder !== null) {
      ...
      const articlePhrase = tryArticleHomophonePhrase(spoken, spokenIdx, expectedWord);
      if (articlePhrase) {
        const spokenDisplay = alignmentSpokenDisplay(
          tokens,
          tokenIndex,
        ...
        spokenIdx += articlePhrase.wordsConsumed;
        digitRemainder = null;
        matched = true;
        break;
      }
