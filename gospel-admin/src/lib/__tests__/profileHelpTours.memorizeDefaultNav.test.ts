...
const RESUME_KEY = 'gospel-memorize-tour-resume-v1'

describe('memorize tour — default profile navigation', () => {
  ...
  it('tryStartMemorizeTourAfterNavigation consumes storage and starts tour on default slug', () => {
    sessionStorage.setItem(RESUME_KEY, JSON.stringify({ v: 1, captiveForTour: false }))

    tryStartMemorizeTourAfterNavigation('default')

    expect(sessionStorage.getItem(RESUME_KEY)).toBeNull()
    expect(driver).toHaveBeenCalled()
  })

  it('tryStartMemorizeTourAfterNavigation no-ops for non-default slug', () => {
    sessionStorage.setItem(RESUME_KEY, JSON.stringify({ v: 1, captiveForTour: false }))

    tryStartMemorizeTourAfterNavigation('other-slug')
