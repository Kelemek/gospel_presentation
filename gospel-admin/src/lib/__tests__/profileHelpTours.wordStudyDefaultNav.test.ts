...
const RESUME_KEY = 'gospel-word-study-tour-resume-v1'

describe('word study tour — default profile navigation', () => {
  ...
  it('stores continueFullWalkthroughAt when captive (full walkthrough)', () => {
    ...
    runWordStudyFeatureTour({ captive: true })
  ...
  it('tryStartWordStudyTourAfterNavigation consumes storage and starts tour on default slug', () => {
    sessionStorage.setItem(RESUME_KEY, JSON.stringify({ v: 1, captiveForTour: false }))

    tryStartWordStudyTourAfterNavigation('default')

    expect(sessionStorage.getItem(RESUME_KEY)).toBeNull()
    expect(driver).toHaveBeenCalled()
  })

  it('tryStartWordStudyTourAfterNavigation no-ops for non-default slug', () => {
    sessionStorage.setItem(RESUME_KEY, JSON.stringify({ v: 1, captiveForTour: false }))

    tryStartWordStudyTourAfterNavigation('other-slug')
