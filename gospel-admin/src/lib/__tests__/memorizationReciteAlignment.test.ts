...
import { isSingleVerseScriptureReference } from '@/lib/parse-scripture-reference';
  ...
  describe('reference digit strictness', () => {
    ...
    it('marks wrong verse numbers as incorrect (Romans 8:28)', () => {
      for (const transcript of [
        ...
        expect(verseTens?.status).toBe('correct');
        expect(verseOnes?.status).toBe('wrong');
        expect(summary.wrongCount).toBeGreaterThan(0);
        expect(summary.correctCount).toBeLessThan(romansTypable.length);
      }
    });
    ...
    it('aligns numbered book prefixes spoken as ordinals (2 Timothy)', () => {
      ...
      for (const transcript of [
        ...
        expect(summary.correctCount).toBe(timothyTypable.length);
        expect(summary.wrongCount).toBe(0);
        expect(summary.missingCount).toBe(0);
      }
    ...
    it('aligns verse number words like twelve (James 1:1) without marking them skipped', () => {
      ...
      for (const transcript of [`${verse} james 1 1`, `${verse} james 1:1`]) {
        ...
        expect(twelve?.status).toBe('correct');
        expect(twelve?.spokenText).toBe('twelve');
        expect(formatReciteSkippedLabels(jamesTokens, summary.results)).not.toContain(
          'twelve'
        );
        expect(summary.missingCount).toBe(0);
      }
    ...
    it('accepts Ascents when STT hears a sense (Psalm superscription)', () => {
      const tokens = buildMemorizationTokens(
      ...
      const summary = alignRecitation(
        tokens,
        typable,
        'a song of a sense of david o lord my heart is not lifted up psalm 131 1',
        'Psalm 131:1'
      );
      const ascents = summary.results.find(
        (r) => normalizeReciteWord(tokens[r.tokenIndex]?.text ?? '') === 'ascents'
      );
      expect(ascents?.status).toBe('correct');
    });
