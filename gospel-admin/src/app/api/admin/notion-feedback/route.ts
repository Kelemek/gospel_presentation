...
type AdminSettingsWriter = {
  update: (values: AdminSettingsUpdate) => {
    eq: (column: 'id', value: number) => Promise<{ error: { message: string } | null }>
  }
}
...
export async function GET() {
  ...
  try {
    ...
    const storedToken = row?.notion_token?.trim() || null
    const storedDatabaseId = row?.notion_database_id ? normalizeNotionId(row.notion_database_id) : ''
    return NextResponse.json(
      publicSettingsPayload(row?.notion_feedback_enabled === true, storedDatabaseId, storedToken)
    )
  } catch (error) {
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const currentToken = asNotionFeedbackConfigRow(existing)?.notion_token?.trim() || null
    ...
    const { error: updateError } = await (admin.from('admin_settings') as unknown as AdminSettingsWriter)
