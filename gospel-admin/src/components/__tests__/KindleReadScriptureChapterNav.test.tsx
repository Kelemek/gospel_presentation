  ...
  it('renders prev and next chapter links', () => {
    render(
      <KindleReadScriptureChapterNavLinks
        nav={{
          prev: { href: '/read/scripture/?ref=Acts+19&from=bxrp', label: 'Previous (Acts 19)' },
          next: { href: '/read/scripture/?ref=Acts+21&from=bxrp', label: 'Next (Acts 21)' },
        }}
      />
    )
    expect(screen.getByRole('link', { name: 'Previous (Acts 19)' })).toHaveAttribute(
      'href',
      '/read/scripture/?ref=Acts+19&from=bxrp'
    )
    expect(screen.getByRole('link', { name: 'Next (Acts 21)' })).toHaveAttribute(
      'href',
