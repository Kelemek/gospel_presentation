...
import { useChapterStreamingAudioListen, PLAYLIST_ADVANCE_LOAD_FALLBACK_MS } from '@/hooks/useChapterStreamingAudioListen'
import {
...
describe('useChapterStreamingAudioListen', () => {
  ...
  it('starts the next playlist track if the reader never catches up', async () => {
    jest.useFakeTimers()
    try {
      const urls = [
        '/api/scripture/audio?reference=Genesis%201&translation=esv',
      ...
      render(<Harness audioUrls={urls} enabled playbackReady playlistStartIndex={0} />)
      await user.click(screen.getByRole('button', { name: 'primary' }))
      const el = screen.getByTestId('passage-audio') as HTMLAudioElement
  ...
  it('starts the next playlist track if the next chapter never becomes ready', async () => {
    jest.useFakeTimers()
    try {
      const urls = [
        '/api/scripture/audio?reference=Genesis%201&translation=esv',
      ...
      const user = userEvent.setup({ advanceTimers: jest.advanceTimersByTime })
      const { rerender } = render(
        <Harness audioUrls={urls} enabled playbackReady playlistStartIndex={0} />
      )
      await user.click(screen.getByRole('button', { name: 'primary' }))
      const el = screen.getByTestId('passage-audio') as HTMLAudioElement
      ...
      rerender(
        <Harness audioUrls={urls} enabled playbackReady={false} playlistStartIndex={1} />
      )
      expect(HTMLMediaElement.prototype.play).toHaveBeenCalledTimes(1)

      await act(async () => {
        jest.advanceTimersByTime(PLAYLIST_ADVANCE_LOAD_FALLBACK_MS)
        await Promise.resolve()
      })
      expect(el.src).toContain('Matthew')
      expect(HTMLMediaElement.prototype.play).toHaveBeenCalledTimes(2)
  ...
  it('cancels a pending playlist advance when Pause is pressed while waiting', async () => {
    const urls = [
      '/api/scripture/audio?reference=Genesis%201&translation=esv',
      '/api/scripture/audio?reference=Matthew%201&translation=esv',
    ]
    ...
    rerender(
      <Harness audioUrls={urls} enabled playbackReady playlistStartIndex={1} />
    )
    await act(async () => {
      await Promise.resolve()
    })
