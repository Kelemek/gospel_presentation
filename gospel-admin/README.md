...
**Deploy update notice** polls for missed release notes on web and native; the native app also asks users to close and reopen when a deploy lands mid-session. For user-visible releases, **append** the same plain-language note to **`data/deploy-update-changelog.json`** (What's new) and **`data/site-changelog.json`** (Help → Change log)—**never delete or trim** older deploy entries (clients track read progress by array index). Preferred: `npm run append-release-changelog -- "Your note."` (see `.cursor/rules/deploy-update-message.mdc`). The JSON file keeps full history; each alert shows at most **five** unseen notes (client-side only). Web-only—no App Store / Play update required for notice text.
