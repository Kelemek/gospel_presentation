/**
 * Import 2 Peter COMA Study template into the database
 */
...
async function import2PeterTemplate() {
  try {
    const templatePath = path.join(__dirname, '../data/templates/2peter-coma-study.json');
    const templateData = JSON.parse(fs.readFileSync(templatePath, 'utf8'));

    const slug = '2petcoma' + Date.now().toString().slice(-6);

    console.log('Importing 2 Peter COMA Study template...');
    ...
    const { data: existing } = await supabase
      ...
      .ilike('title', '%2 Peter COMA%')
      .eq('is_template', true);

    if (existing && existing.length > 0) {
      console.log('\nExisting 2 Peter templates found:');
