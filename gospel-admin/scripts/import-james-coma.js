/**
 * Import James COMA Study template into the database
 */
...
async function importJamesTemplate() {
  try {
    const templatePath = path.join(__dirname, '../data/templates/james-coma-study.json');
    const templateData = JSON.parse(fs.readFileSync(templatePath, 'utf8'));

    const slug = 'jamescoma' + Date.now().toString().slice(-6);

    console.log('Importing James COMA Study template...');
    ...
    const { data: existing } = await supabase
      ...
      .ilike('title', '%James COMA%')
      .eq('is_template', true);

    if (existing && existing.length > 0) {
      console.log('\nExisting James templates found:');
