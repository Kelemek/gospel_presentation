/**
 * Import Ephesians COMA Study template into the database
 */
...
async function importEphesiansTemplate() {
  try {
    const templatePath = path.join(__dirname, '../data/templates/ephesians-coma-study.json');
    const templateData = JSON.parse(fs.readFileSync(templatePath, 'utf8'));

    const slug = 'ephcoma' + Date.now().toString().slice(-6);

    console.log('Importing Ephesians COMA Study template...');
    ...
    const { data: existing } = await supabase
      ...
      .ilike('title', '%Ephesians COMA%')
      .eq('is_template', true);

    if (existing && existing.length > 0) {
      console.log('\nExisting Ephesians templates found:');
