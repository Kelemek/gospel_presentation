...
- **Deploy update notice.** Optional end-user changelog for web and native “what’s new” / restart alerts: append to `gospel-admin/data/deploy-update-changelog.json` in user-visible releases (plain language; see `.cursor/rules/deploy-update-message.mdc`). Shipped with each Vercel deploy via `GET /api/app-deploy-version`.
...
- **Automated DB backups** exclude re-importable CCEL corpus profiles (`sg`, `me`, `cv`, `mh`, `je`, `jefow`/`jerea`/`jetog`, `lgal`, `ltbw`, `ppgr`, `aogr`, `bxrp`, `jryh`, `lbst`, `twcm`/`twbt`/`twbd`/`twdc`/`twlp`/`twtc`) and their `spurgeon_passage_index` rows; disaster recovery for those corpora is the import scripts above, not Storage restore. Purge bloated `db-backups`: `node scripts/purge-db-backups-bucket.js` (see [backups/README.md](backups/README.md)).
