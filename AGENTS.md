...
- **PostHog (monitoring).** Optional for local dev: `NEXT_PUBLIC_POSTHOG_KEY`, `NEXT_PUBLIC_POSTHOG_HOST` (e.g. `https://us.i.posthog.com`). When unset, analytics does not load (CI/tests are unaffected). Replaces former Sentry + Clarity env vars.
