...
- **CCEL Edwards sermons.** Import Jonathan Edwards *Select Sermons* from `gospel-admin/`: `npm run import-edwards` (optional `--parse-only`, `--dry-run`, `--purge-je`). Creates public profiles `je01`–`je19` and `spurgeon_passage_index` rows. Add **Edwards library** in Admin → Settings and **Save order** for the Resources menu.
- **STEP Bible word study.** Scripture modal **Words** toggle uses JSON under `gospel-admin/data/stepbible/` (no DB). After clone or STEPBible-Data updates: `npm run import-stepbible` (full Bible; downloads ~100MB+ from GitHub). Tests/minimal dev: `npm run import-stepbible:fixtures`.
