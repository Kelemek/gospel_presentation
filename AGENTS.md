...
- **Native app deploy notice.** Optional end-user changelog for the Capacitor restart alert: edit `gospel-admin/data/deploy-update-message.txt` in user-visible releases (plain language; see `.cursor/rules/deploy-update-message.mdc`). Shipped with each Vercel deploy via `GET /api/app-deploy-version`.
