...
- **Automated DB backups** exclude re-importable CCEL corpus profiles (`sg`, `me`, `cv`, `mh`, `je`, `jefow`/`jerea`/`jetog`, `lgal`, `ppgr`, `aogr`, `bxrp`, `jryh`, `lbst`, `twcm`/`twbt`/`twbd`/`twdc`/`twlp`/`twtc`) and their `spurgeon_passage_index` rows; disaster recovery for those corpora is the import scripts above, not Storage restore. Purge bloated `db-backups`: `node scripts/purge-db-backups-bucket.js` (see [backups/README.md](backups/README.md)).
