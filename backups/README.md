...
**Table discovery:**
- Uses `get_backup_tables()` to dynamically include new `public` tables (excludes cache/log/session tables, `backup_runs`, `bible_verses`, `scripture_access_logs`, `verification_codes`, ephemeral device-sync tables `pairing_sessions` and `sync_pairing_claim_rate_limits`; apply **`sql/migrations/20260509_get_backup_tables_fix_log_exclusions.sql`** if an older migration still listed the wrong log table name and pulled full access logs into backups). **`sync_key_entries`** (encrypted device-sync payloads) is included and uses `updated_at` for differential exports.
